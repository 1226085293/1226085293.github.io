...
<AutoSizeImage src="https://forum.cocos.org/uploads/default/optimized/3X/3/2/32af8f6d554f39f1d90757c9355e5db51b476dc8_2_690x347.jpeg =690x347" />
...
<AutoSizeImage src="https://s1.imagehub.cc/images/2023/08/14/F3SVZU_91YZOK8OE.png" />

### 自动构建

使用 github actions，可以直接从 github 同步至 gitee，你只需要上传你的文章，github actions 自动构建并同步至 gitee pages，简直不要太方便

<AutoSizeImage src="https://s1.imagehub.cc/images/2023/08/14/imagecb3d0f94a91b9f1d.png" />
    ...
    let convert_string_f = (value_s_: string, connect_b_ = false) => {
      ...
      while (true) {
        ...
        switch (closest_index_n) {
          ...
          case 1: {
            result_ss.push(
              `<AutoSizeImage src="${match_result[2]} =${match_result[4]}x${match_result[5]}" />`
            )
            break
          }
          // 图片
          case 10: {
            result_ss.push(
              `<AutoSizeImage src="${match_result[1]} =${match_result[3]}x${match_result[4]}" />`
            )
            break
          }
          // 引用
